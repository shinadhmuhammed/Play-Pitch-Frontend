...
function Activities() {
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        const response = await axiosUserInstance.post("/getActivity", {
          userId,
        });
          ...
          <div className="mr-96 p-20 w-full ml-16">
            ...
            {activities && activities.length > 0 ? (
              activities.map((activity) => (
                <div
                  key={activity._id}
                  className="mb-8 p-4 rounded-lg shadow-lg bg-white"
                >
                  ...
                  <p>
                    <span className="font-bold">Slot:</span> {activity.slot}
                  </p>
                  <p>
