...
function VenueRequest() {
    ...
    const handleAccept = async (turfId: string,turfOwnerEmail:string) => {
        try {
            await axiosAdminInstance.post('/admin/venueaccept', { turfId, turfOwnerEmail });
            setTurfs(prevTurfs => prevTurfs.map(turf => {
                if (turf._id === turfId) {
                    return { ...turf, isActive: true };
                }
                return turf;
            }));
    ...
    const handleDecline = async (turfId: string) => {
        try {
            // Handle decline logic, e.g., sending request to backend to mark turf as declined
        ...
        <div>
            ...
            {turfs.map((turf) => (
                <div key={turf._id} className="border p-4 mb-4">
                    ...
                    <div className="mt-4">
                        {/* Conditionally render "Accepted" label or "Accept" button */}
                        {turf.isActive ? (
                            <span className="text-green-600 font-bold">Accepted</span>
                        ) : (
                            <button onClick={() => handleAccept(turf._id, turf.turfOwnerEmail)} className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 mr-2">Accept</button>

                        )}

                        {/* Render "Decline" button if turf is not accepted */}
                        {!turf.isActive && (
                            <button onClick={() => handleDecline(turf._id)} className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4">Decline</button>
                        )}
                    </div>
                </div>
            ))}
